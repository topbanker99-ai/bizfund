...
import { botcheckEnabled, verifyPow, issueTicket, verifyTicket } from './_lib/botcheck.js';
...
export default async function handler(req, res) {
  ...
  // ── 봇 차단(작업증명) ── PoW 1회 검증 → 세션 티켓 발급 → 이후 호출은 티켓 재사용.
  if (action === 'ticket') {
    if (botcheckEnabled() && !verifyPow(body.pow || req.headers['x-pow'] || '')) {
      return res.status(403).json({ error: '사람 확인에 실패했어요. 새로고침 후 다시 시도해주세요.' });
    }
    return res.status(200).json({ ticket: issueTicket('vi') });
  }
  if (botcheckEnabled() && !verifyTicket(req.headers['x-vi-ticket'] || body.ticket || '', 'vi')) {
